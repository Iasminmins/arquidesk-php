...
$queries = [
    'projects' => ['sql' => 'select client_name, client_phone, project_name, current_stage, project_status, closed_value, entry_date, closing_date, notes from client_projects where company_id = ? order by updated_at desc', 'params' => [$companyId], 'label' => 'Projetos'],
    'future_clients' => ['sql' => 'select fc.name, fc.phone, fc.email, fc.address, fc.interest, fc.estimated_value, fc.contact_date, fc.next_contact_date, fc.source, fc.status, fc.notes, u.name as responsavel from future_clients fc left join users u on u.id = fc.designer_id where fc.company_id = ? order by fc.created_at desc', 'params' => [$companyId], 'label' => 'Clientes Futuros'],
    'finance' => ['sql' => 'select client_name, project_name, sold_value, payment_method, sale_date, notes from financial_sales where company_id = ? order by sale_date desc', 'params' => [$companyId], 'label' => 'Vendas'],
    'payments' => ['sql' => 'select s.client_name, s.project_name, p.payment_number, p.amount, p.payment_date, s.payment_method from financial_payments p join financial_sales s on s.id = p.financial_sale_id where p.company_id = ? order by p.payment_date desc', 'params' => [$companyId], 'label' => 'Pagamentos'],
    'goals' => ['sql' => 'select u.name as projetista, g.month as mes, g.year as ano, g.goal_amount as meta from designer_goals g join users u on u.id = g.designer_id where g.company_id = ? order by g.year desc, g.month desc', 'params' => [$companyId], 'label' => 'Metas'],
...
if ($type === 'stage') {
    ...
    $stmt = db()->prepare(
        'select client_name, client_phone, project_name, current_stage, project_status, negotiation_status, conference_status, assembly_status, assistance_status, closed_value, entry_date, presentation_date, closing_date, sent_to_factory_date, billing_date, assembly_started_date, assembly_finished_date, assistance_date, order_date, notes
         from client_projects where company_id = ? and current_stage = ? order by updated_at desc'
    );
