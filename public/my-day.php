...
$autoGenerated = generate_daily_auto_items($companyId, $date, $employees);
...
$itemsSql = "select d.*, p.client_name, p.project_name, p.current_stage, u.name as user_name, u.role as user_role
    ...
    where d.company_id = ? and d.checklist_date = ?";
$itemsParams = [$companyId, $date];
if ($isAdmin && $selectedUserId > 0) {
...
$itemsSql .= " order by field(d.status, 'PENDENTE','CONCLUIDO','CANCELADO'), field(d.priority, 'ALTA','NORMAL','BAIXA'), d.source desc, d.created_at desc";
...
<section class="grid gap-5">
    ...
    <section class="overflow-hidden rounded-lg border border-line bg-white">
        <div class="grid gap-4 border-b border-line bg-ink p-5 text-white lg:grid-cols-[1fr_auto] lg:items-end">
            <div>
                <span class="text-xs font-bold uppercase tracking-wide text-white/60">Rotina inteligente</span>
                <h2 class="mt-2 text-2xl font-black">Meu Dia</h2>
            ...
            <form method="get" class="grid w-full min-w-0 gap-2 rounded-lg border border-white/10 bg-white/10 p-3 backdrop-blur sm:grid-cols-[minmax(0,160px)_minmax(0,220px)_auto] sm:items-end">
                ...
                <?php endif; ?>
                <button class="min-h-10 rounded-md bg-white px-4 text-sm font-black text-ink hover:bg-fog" type="submit">Filtrar</button>
            </form>
        </div>
        <div class="grid gap-3 p-4 md:grid-cols-4">
            ...
            <div class="rounded-md border border-line bg-white p-4">
                <span class="text-sm text-slate-500">Total do dia</span>
                <strong class="mt-2 block text-3xl"><?= (int) $stats['TOTAL'] ?></strong>
            </div>
        </div>
    </section>

    <div class="grid min-w-0 gap-5 xl:grid-cols-[minmax(0,380px)_minmax(0,1fr)]">
        <section class="min-w-0 rounded-lg border border-line bg-white p-4">
            <div class="flex items-center justify-between gap-3">
                ...
                <span class="rounded-full border border-indigo-100 bg-indigo-50 px-3 py-1 text-xs font-bold text-indigo-700">Editável</span>
            </div>
            <form method="post" class="mt-4 grid gap-3">
                <?= csrf_field() ?>
                <input type="hidden" name="action" value="create">
                <input type="hidden" name="checklist_date" value="<?= e($date) ?>">
                <?php if ($isAdmin): ?>
                ...
                <button class="min-h-10 w-full min-w-0 rounded-md bg-ink px-4 text-sm font-bold text-white hover:opacity-95" type="submit">Adicionar ao dia</button>
            </form>
        </section>
        ...
        <section class="min-w-0 overflow-hidden rounded-lg border border-line bg-white">
            <div class="flex flex-col gap-1 border-b border-line p-4 sm:flex-row sm:items-center sm:justify-between">
                <div>
                    <h3 class="font-bold">Itens de <?= e(date_br($date)) ?></h3>
            ...
            <div class="divide-y divide-line">
                <?php if (!$items): ?>
                    <div class="p-6 text-sm text-slate-500">Nenhum item criado para esta data.</div>
                <?php endif; ?>
