...
$targetStage = $_POST['target_stage'] ?? 'NEGOCIACAO';
...
if (!in_array($targetStage, ['PROJETO', 'NEGOCIACAO'], true)) {
...
db()->prepare('update client_projects set current_stage = ?, negotiation_status = ? where id = ? and company_id = ?')
    ->execute([$targetStage, $target !== '' ? $target : null, $id, $companyId]);
